...
import { Building2, Plus, Pencil, Trash2, Search, X, Phone, Mail, MapPin } from "lucide-react";
...
export default function VendorsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      editing ? await vendorsApi.update(editing.id, form) : await vendorsApi.create(form);
      setFormOpen(false);
      fetchAll();
    } catch (err: unknown) {
  ...
  const handleDelete = async () => {
    ...
    try { await vendorsApi.delete(deleteTarget.id); setDeleteTarget(null); fetchAll(); }
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={canEdit ? <Button onClick={openCreate} size="sm"><Plus size={14} /> New Vendor</Button> : undefined}
      />
      ...
      <div className="relative mb-4 max-w-sm">
        ...
        <input
          value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search vendors…"
      ...
      {filtered.length === 0 ? (
        <EmptyState
          icon={Building2} title="No vendors found"
          description={search ? "Try a different search term" : "Add your first vendor"}
          action={canEdit && !search ? <Button size="sm" onClick={openCreate}><Plus size={14} /> New Vendor</Button> : undefined}
        />
      ) : (
                  ...
                  <tr key={v.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="space-y-1">
                        {v.email && <div className="flex items-center gap-1.5 text-xs text-gray-600"><Mail size={11} className="text-gray-400" />{v.email}</div>}
                        {v.phone && <div className="flex items-center gap-1.5 text-xs text-gray-600"><Phone size={11} className="text-gray-400" />{v.phone}</div>}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      {v.address ? (
                    ...
                    {canEdit && (
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => openEdit(v)} className="p-1.5 rounded-lg text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors"><Pencil size={13} /></button>
                          <button onClick={() => setDeleteTarget(v)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors"><Trash2 size={13} /></button>
                        </div>
                      </td>
                    )}
                  </tr>
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)} title={editing ? "Edit Vendor" : "New Vendor"} size="md">
        <form onSubmit={handleSubmit} className="space-y-4">
          {formError && <Alert variant="error">{formError}</Alert>}
          <Input label="Name *" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Vendor / supplier name" required />
          <Input label="Email" type="email" value={form.email ?? ""} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="vendor@example.com" />
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              rows={2} placeholder="Street, City, State, PIN"
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <Button type="submit" size="sm" loading={formLoading}>{editing ? "Update" : "Create Vendor"}</Button>
          </div>
        </form>
      </Modal>
