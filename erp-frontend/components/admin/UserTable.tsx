...
import { adminApi, type UserDTO } from "@/lib/api/admin.api";
import { Alert } from "@/components/ui/Alert";
import { ALL_ROLES, roleLabel, roleBadgeClass } from "@/lib/utils/roles";
...
export function UserTable({ users, onRefresh }: UserTableProps) {
  ...
  const toggleStatus = (u: UserDTO) =>
    withLoading(u.id, () =>
      adminApi.setUserStatus(u.id, !u.enabled).then(() => {})
    );
  ...
  const deleteUser = (u: UserDTO) => {
    if (!confirm(`Delete ${u.email}? This cannot be undone.`)) return;
    withLoading(u.id, () => adminApi.deleteUser(u.id).then(() => {}));
  };
          ...
          <thead>
            <tr className="border-b border-gray-200">
              {["User", "Roles", "Status", ""].map((h) => (
                <th
                  key={h}
                  className="pb-3 pr-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide last:pr-0 last:text-right"
                >
                  {h}
                </th>
              ))}
                    ...
                    {isEditing ? (
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {u.roles.map((r) => (
                          <span
                            key={r}
                            className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold ${roleBadgeClass(r as never)}`}
                          >
